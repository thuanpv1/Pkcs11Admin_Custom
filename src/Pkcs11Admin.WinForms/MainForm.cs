...
namespace Net.Pkcs11Admin.WinForms
{
    public partial class MainForm : Form
    {
        ...
        private void CopyDriverFilesToSystem()
        {
            ...
            try
            {
                CopyFileIfNotExist(ChipDocLiteToken, destinationFile1);
                CopyFileIfNotExist(ciamd, destinationFile2);
                CopyFileIfNotExist(lasermd, destinationFile3);
                CopyFileIfNotExist(ChipDocLiteToken, destinationFile21);
                CopyFileIfNotExist(ciamd, destinationFile22);
                CopyFileIfNotExist(lasermd, destinationFile23);
            }
        ...
        private void CopyFileIfNotExist(String sourFileName, String desFileName)
        {
            if (File.Exists(desFileName) && FileCompare(sourFileName, desFileName))
            {
                return;                
            }

            File.Copy(sourFileName, desFileName, true);
